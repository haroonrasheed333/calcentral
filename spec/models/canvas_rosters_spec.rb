...
describe "CanvasRosters" do
  it "should return a list of officially enrolled students for a unlinked Canvas course site" do
    ...
    CampusData.stub(:get_enrolled_students).with(linked_section_ccn, '2013', 'C').and_return(
        [
            {
    ...
    model = CanvasRosters.new(teacher_login_id, course_id: course_id)
    feed = model.get_feed
  ...
  it "should give access to Canvas course instructors" do
    ...
    model = CanvasRosters.new(user_id, course_id: teaching_site_id)
    model.user_authorized?.should be_true
    model = CanvasRosters.new(user_id, course_id: student_site_id)
  ...
  it "should show official photo links for students who are not waitlisted in all sections" do
    ...
    CampusData.stub(:get_enrolled_students).with(a_section_ccn, '2013', 'C').and_return(
        [
            {
    ...
    CampusData.stub(:get_enrolled_students).with(b_section_ccn, '2013', 'C').and_return(
        [
            {
                'ldap_uid' => enrolled_student_login_id,
                'enroll_status' => 'E',
    ...
    feed[:students].index {|student| student[:id] == enrolled_student_canvas_id &&
        !student[:photo].end_with?(CanvasRosters::PHOTO_UNAVAILABLE_FILENAME)
    }.should_not be_nil
  ...
  it "should only download photo data for officially fully enrolled students" do
    ...
    CampusData.stub(:get_photo).with(enrolled_student_login_id).and_return(
        {
            'bytes' => 42,
            'photo' => photo_data
        }
    ...
    waitlisted_photo[:filename].end_with?(CanvasRosters::PHOTO_UNAVAILABLE_FILENAME).should be_true
    unofficial_photo = model.photo_data_or_file(unofficial_student_canvas_id)
    unofficial_photo[:data].should be_nil
    unofficial_photo[:filename].end_with?(CanvasRosters::PHOTO_UNAVAILABLE_FILENAME).should be_true
  end
